...
Np = np.shape(p5['base'])[0]

found  = 0
for i in range(np.shape(p5['orientations'])[0]):
    yShift = 0
    orientation = p5['orientations'][i]
    typ         = p5['types'][i]
    position    = np.copy(orientation)  
    while np.max(position[...,0])<grid[0]:
        while np.max(position[...,1])<grid[1]:
            newPosition = np.zeros(nCells+Np,dtype=int)
